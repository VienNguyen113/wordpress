...
function popster_related_posts() {
	...
	if($tags) {
        ...
        if($related_posts) {
            ...
            <div class="no_related_post type-3"><?php _e('Cannot Retrieved a Related Posts Yet!', 'popster'); ?></div>
		<?php }
	}
	wp_reset_query();
}
...
function popster_pagenavi(){
	...
	if ( $total > 1 )  {
		...
		echo '<span class="page-of">'. __('Page', 'popster') . ' ' . $current_page . __(' of ', 'popster') . $total . '</span>';
		...
		echo '<nav class="page-navigation">';
		echo '<span class="page-of">'. __('Page 1 of 1', 'popster') . '</span>';
		echo '</nav>';
	}
}
			...
			<?php while($recent->have_posts()) : $recent->the_post();?>
				<li class="post clearfix">
					...
					<article class="boxed">
						...
						<header>
							<div class="category-meta"><?php _e('Featured ', 'popster'); echo $i;  _e(' of ', 'popster'); echo $total_post; ?> </div> 
					...
					<!--<p class="galery-slide-desc"><?php echo popster_get_option('stratify_featured_posts_title') ? popster_get_option('stratify_featured_posts_title') : _e('Featured', 'popster'); ?> <?php echo $i; ?> of <?php echo $total_post; ?></p>-->
...
function popster_relative_time($time = false, $limit = 86400, $format = 'g:i A M jS') {
	...
	if ($time === $now) $relative = __('now', 'popster');
	elseif ($time > $now) $relative = __('in the future', 'popster');
	else {
		$diff = $now - $time;

		/*if ($diff >= $limit) $relative = date($format, $time);
		else*/if ($diff < 60) {
			$relative = __('less than one minute ago', 'popster');
